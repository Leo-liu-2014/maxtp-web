...
//划过显示立即申请等效果
function sqhover(model,obj,box){
	$(model).each(function(index, element) {
		var attr = this;
        $(obj,attr).hover(function(){
			$(box,attr).show().stop().animate({height:"32px"},150)
...
function nomust(obj,num,endfn){
	 ...
	 var len
  	 if(obj.val()==""){
	      if(obj.hasClass("mustVal")){
			    if(num){nokong($(this),num);}//点击确认后显示错误提示
				//yanxf 2015-07-03 
				setInfo({obj:obj,icon:icon,content:"必填项，请填写"});
		  }else{
		  		if(num){num--;}
				setHide(obj,".textError")//隐藏
		  }
	...
	//  yanxf 2015-07-06
			...
			setInfo({obj:obj,icon:icon,content:"请输入小于300的付款金额"});
		 	...
		 	onbginner(obj);
			setInfo({obj:obj,icon:icon,content:"请输入6位数字"});
